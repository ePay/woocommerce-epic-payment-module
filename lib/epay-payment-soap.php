 ...
 */
// class Epay_EPIC_Payment_Soap extends epay_epic_payment_api {
class Epay_EPIC_Payment_Soap {
	...
	public function __construct( $pwd = '', $subscription = false,  $apikey = false, $posid = false) {
        ...
        $this->epay_epic_payment_api = new epay_epic_payment_api($this->apikey, $this->posid);
	}
	...
	public function authorize( $merchantnumber, $subscriptionid, $orderid, $amount, $currency, $instantcapture, $group, $email ) {
        ...
        {
            $currency = Epay_EPIC_Payment_Helper::get_iso_code( $currency, false );
            $instantcapture = ($instantcapture ? "NO_VOID" : "OFF");

            $result_json = $this->epay_epic_payment_api->authorize($subscriptionid, $amount, $currency, (string) $orderid, $instantcapture, $orderid, Epay_EPIC_Payment_Helper::get_epay_payment_callback_url( $orderid ));
	...
	public function delete_subscription( $merchantnumber, $subscriptionid ) {
        ...
        {
            $result = $this->epay_epic_payment_api->delete_subscription($subscriptionid);
	...
	public function capture( $merchantnumber, $transactionid, $amount ) {
        ...
        {
            $result_json = $this->epay_epic_payment_api->capture($transactionid, $amount);
            $result_arr = json_decode($result_json, true);
	...
	public function refund( $merchantnumber, $transactionid, $amount ) {
        ...
        {
            $result_json = $this->epay_epic_payment_api->refund($transactionid, $amount);
            $result_arr = json_decode($result_json, true);
	...
	public function delete( $merchantnumber, $transactionid ) {
        ...
        {
            $result_json = $this->epay_epic_payment_api->void($transactionid);
            $result_arr = json_decode($result_json, true);
	...
	public function get_transaction( $merchantnumber, $transactionid ) {
        ...
        {
            $result_json = $this->epay_epic_payment_api->payment_info($transactionid);
            $result_obj = json_decode($result_json);

            $result = new stdClass;

            if(isset($result_obj->transaction->state) && $result_obj->transaction->state == "SUCCESS")
            {
	...
	public function get_epay_error( $merchantnumber, $epay_response_code ) {
		...
		try {
			...
			$epay_params['merchantnumber']   = $merchantnumber;
			$epay_params['pwd']              = $this->pwd;
			$epay_params['language']         = Epay_EPIC_Payment_Helper::get_language_code( get_locale() );
